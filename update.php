...
if(($_POST)>0){
	...
	$quizid=mysqli_query($conn,"SELECT quizid FROM quiz WHERE quizid='$id'");
	if(!$quizid){
	...
	$detail = mysqli_query($conn,"SELECT * FROM questions where quizid='$sql'");
	while($row = $detail->fetch_assoc()){
		echo'<label for="question">Enter the question: <em>&#x2a;</em></label><input id="question" value="'; echo $row["questions"]; echo'" name="question" type="text" </label>';
		$q= $row['qid'];
		// echo $rows['qid'].'<br>';
		 $result= mysqli_query($conn,"SELECT * FROM options where qid='$q'");
		 while ($rows = $result->fetch_assoc()){
	
			// echo $rows["options"].'<br>';
	echo '<label for="optiona">Option: <em>&#x2a;</em></label><input id="optiona" value="'; echo $rows["options"]; echo'" name="optiona" required="" type="text" />';
		 }
	/* echo '<label for="optionb">Option B: <em>&#x2a;</em></label><input id="optionb" name="optionb" required="" type="text" />
	echo '<label for="optionb">Option B: <em>&#x2a;</em></label><input id="optionb" value="'; echo $rows["options"]; echo'" name="optionb" required="" type="text" />';
	    <label for="optionc">Option C: <em>&#x2a;</em></label><input id="optionc" name="optionc" required="" type="text" />
		<label for="optiond">Option D: <em>&#x2a;</em></label><input id="optiond" name="optiond" required="" type="text" />
		<label for="ca">Correct answer: <em>&#x2a;</em></label><input id="ca" name="ca" required="" type="text" />
		<label> ★━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━★</label>'; */
		echo '<label for="ca">Correct answer: <em>&#x2a;</em></label><input id="ca" value="'; echo $row["ca"]; echo'" name="ca" required="" type="text" />';
		echo '<label> ★━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━★</label>';
 ...
 //{
   /* echo '<br>
	...
	echo '<a class="button" name="submit" id="submit" href="upquiz.php?quizid='; echo("$sql"); echo'"><center>UPDATE</center></a>
  </form>
</div>';
//}
}
