...
if(isset($_POST["submit"])){
	...
	$quizid=mysqli_query($conn,"SELECT quizid FROM quiz ORDER BY quizid='$id'");
	if(!$quizid){
	...
	$noq= mysqli_query($conn,"SELECT noq from quiz where quizid ='$quiz'");
	if(!$noq){
	die(mysqli_error($conn));
	}
	...
	/*$question = $conn->real_escape_string($_REQUEST[$i.'question']); 
    $optiona = $conn->real_escape_string($_REQUEST[$i.'optiona']); 
    $optionb = $conn->real_escape_string($_REQUEST[$i.'optionb']); 
    $optionc = $conn->real_escape_string($_REQUEST[$i.'optionc']);
	$optiond = $conn->real_escape_string($_REQUEST[$i.'optiond']);
	$ca = $conn->real_escape_string($_REQUEST[$i.'ca']); */
	
// Attempt update query execution
    
    $sql = mysqli_query($conn,"UPDATE questions SET ('$quiz',DEFAULT,'$question','$ca')");
	//echo("Successful!!");
	
/*	$qid=mysqli_query($conn,"SELECT qid FROM questions ORDER BY qid DESC LIMIT 1");
	if(!$qid){
	die(mysqli_error($conn));
	}
	if(mysqli_num_rows($qid) > 0) { 
    $qr = mysqli_fetch_assoc($qid);
	//echo $qr["qid"].'<br>';
	$q= $qr["qid"];
	}
	
    $sql1 = mysqli_query($conn,"INSERT INTO options VALUES ('$q','$optiona',DEFAULT)");
	$sql2 = mysqli_query($conn,"INSERT INTO options VALUES ('$q','$optionb',DEFAULT)");
	$sql3 = mysqli_query($conn,"INSERT INTO options VALUES ('$q','$optionc',DEFAULT)");
	$sql4 = mysqli_query($conn,"INSERT INTO options VALUES ('$q','$optiond',DEFAULT)");
	}
